...
try:
    ...
    # ローカル環境等で secrets.toml が存在しない場合はエラーを無視して次へ
    pass
...
with tab1:
    ...
    # --- 最終更新日時の計算と表示 ---
    if not df_posts.empty and 'post_date_dt' in df_posts.columns and not df_posts['post_date_dt'].dropna().empty:
        latest_dt = df_posts['post_date_dt'].max()
        ampm = "AM" if latest_dt.hour < 12 else "PM"
        last_updated_str = latest_dt.strftime(f"%Y.%m.%d.{ampm}")
        update_text = f"🔄 最終更新: {last_updated_str}"
    else:
        update_text = "🔄 最終更新: データなし"
        
    st.markdown(f"<div style='margin-top: -10px; margin-bottom: 15px;'><span style='color: gray; font-size: 0.9rem;'>{update_text}</span></div>", unsafe_allow_html=True)
    # --------------------------------
